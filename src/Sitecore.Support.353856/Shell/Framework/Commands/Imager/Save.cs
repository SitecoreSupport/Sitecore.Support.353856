namespace Sitecore.Support.Shell.Framework.Commands.Imager
{
    using Sitecore;
    ...
    public class Save : Command
    {
        ...
        public override CommandState QueryState(CommandContext context)
        {
            ...
            if (Sitecore.Security.Accounts.User.Current.Profile.ContentLanguage.Length != 0)
            {
                Language currentContentLanguage = LanguageManager.GetLanguage(Sitecore.Security.Accounts.User.Current.Profile.ContentLanguage);
                if (!CanWriteLanguageForCurrentContentLanaguage(currentContentLanguage))
                {
                    return CommandState.Disabled;
                }
            }
        ...
        private bool CanWriteLanguageForCurrentContentLanaguage(Language currentContentLanguage)
        {
            if (!Settings.CheckSecurityOnLanguages || Context.IsAdministrator)
            {
                return true;
            }
            Item languageItem = this.GetLanguageItem(Sitecore.Context.ContentDatabase, currentContentLanguage);

            if (languageItem != null)
            {
                return AuthorizationManager.IsAllowed(languageItem, AccessRight.LanguageWrite, Sitecore.Security.Accounts.User.Current);
            }
            return false;
        }

        private Item GetLanguageItem(Database database, Language itemLanguage)
        {
            ID languageItemId = LanguageManager.GetLanguageItemId(itemLanguage, database);
            if ((object)languageItemId != null)
            {
                return database.GetItem(languageItemId, itemLanguage, Sitecore.Data.Version.Latest);
            }
            return null;
        }
    }
